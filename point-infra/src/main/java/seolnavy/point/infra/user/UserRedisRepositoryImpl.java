...
public class UserRedisRepositoryImpl implements UserRedisRepository {
	...
	private final RedisTemplate<String, Long> remainPointRedisTemplate;

	@Override public void saveRemainPoint(final Long userNo, final Long remainPoint) {
		remainPointRedisTemplate.opsForValue().set(getUserRemainPointKey(userNo), remainPoint);
	}
	...
	@Override public boolean existsRemainPointKey(final Long userNo) {
		return remainPointRedisTemplate.hasKey(getUserRemainPointKey(userNo));
	}
